---
title: "Making Northern Irish CoM tables"
author: "Dave"
format: html
editor: visual
---
...
com <- filter(com, nation == "northern-ireland")
...
read_com_data <- function(x){
  ...
  census_file <-
    here("data", "com-stats",
         str_c("northern-irish-com", x, "2021", sep = "_") |>
           str_c(".csv"))
...
read_diocesan_data <- function(x){
  ...
  data_file <-
    here("data", "diocesan-stats",
         str_c("ni-dioceses", x, "2021", sep = "_") |> 
           str_c(".csv"))
...
national_data <-
  map(national_data, \(x) summarise(x, across(where(is.numeric), sum)))

national_data <-
  map(national_data,
      \(x){
        mutate(x, nation = "northern-ireland") |> 
          relocate(nation)
      })
...
all_data <- 
  ...
  mutate(nation_label = str_to_nice_title(nation)) |> 
...
saveRDS(all_data,
        file.path(save_dir, "northern-ireland_all-com-data_no-relative-counts.RDS"))

vroom_write(all_data, 
      file.path(save_dir, "northern-ireland_all-com-data_no-relative-counts.csv"),
      delim = ",")
```
